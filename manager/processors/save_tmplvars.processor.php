...
switch ($_POST['mode']) {
    case '300':
		...
		$sql = "INSERT INTO " . $modx->getFullTableName('site_tmplvars') . " (name, description, caption, type, elements, default_text, display, display_params, rank, locked, category) VALUES('$name', '$description', '$caption', '$type', '$elements', '$default_text', '$display', '$params', '$rank', '$locked', $categoryid);";
	...
	if($use_udperms==1) {
		...
		if(is_array($docgroups)) {
			foreach ($docgroups as $dgkey=>$value) {
				$sql = "INSERT INTO " . $modx->getFullTableName('site_tmplvar_access') . " (tmplvarid,documentgroup) 
